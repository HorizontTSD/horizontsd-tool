...
export function annotationsPlugin(opts) {
    ...
    function placeMark(uPlot, mark, opts) {
        ...
        Object.assign(markEl.style, {
            ...
            borderRight:
                mark.to > mark.from ? `${opts.width}px ${opts.dash ? "dashed" : "solid"} ${opts.stroke}` : null,
            ...
            drawClear: [
                (uPlot) => {
                    ...
                    xMarks.forEach((mark) => {
                        ...
                        if (
                            (mark.from >= uPlot.scales.x.min && mark.from <= uPlot.scales.x.max) ||
                            (mark.to >= uPlot.scales.x.min && mark.to <= uPlot.scales.x.max) ||
                            (mark.from <= uPlot.scales.x.min && mark.to >= uPlot.scales.x.max)
                        ) {
        ...
        u.series.forEach((s, seriesIdx) => {
            ...
            if (valIdx != null) {
                ...
                if (val != null) {
                    ...
                    el.style.transform =
                        ori == 0 ? `translateX(-50%) translateX(${pos}px)` : `translateY(-50%) translateY(${pos}px)`
...
export function seriesBarsPlugin(opts) {
    ...
    return {
        hooks: {
            init: (u) => {
                for (let el of u.root.querySelectorAll(".u-cursor-pt")) el.style.borderRadius = "unset"
            },
            drawClear: (u) => {
 ...
 */
export function box_whisker__columnHighlightPlugin({
    className,
...
export function box_whisker__legendAsTooltipPlugin({
    className,
...
export function candlestick_ohlc__columnHighlightPlugin({
    className,
    style = { backgroundColor: "rgba(51,204,255,0.3)" },
} = {}) {
    let underEl, overEl, highlightEl, currIdx
...
export function candlestick_ohlc__legendAsTooltipPlugin({
    className,
    style = { backgroundColor: "rgba(255, 249, 196, 0.92)", color: "black" },
} = {}) {
    let legendEl
