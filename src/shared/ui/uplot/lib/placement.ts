...
export default function (anchor: HTMLElement, overlay: HTMLElement, options: PlacementOptions = {}) {
    // Safe style reset
    if (overlay?.style) {
    ...
    const offsetParentComputed = offsetParent
        ? getComputedStyle(offsetParent)
        : {
              borderTopWidth: "0px",
              borderLeftWidth: "0px",
          }
    const overlayComputed = overlay
    ...
    if (overlay.style) {
        ...
        overlay.style[oppositeSide] = `${
            ...
            (parseInt(offsetParentComputed[`border${PROPS[axisSide].Start}Width`]) || 0)
        }px`
        ...
        if (options.bound !== false) {
            ...
            alignPos = Math.max(
                ...
                Math.min(
                    alignPos,
                    factor *
                        (boundRect[PROPS[axisAlign][oppositeAlign]] - offsetParentRect[PROPS[axisAlign][fromAlign]]) -
                        overlaySize
                )
            )
        }
