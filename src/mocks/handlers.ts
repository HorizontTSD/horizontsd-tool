    ...
    http.post<{ sensor_ids: string[] }>(`${API_URL}/backend/v1/get_forecast_data`, async ({ request }) => {
        ...
        sensor_ids.forEach((sensorId) => {
            ...
            response[sensorId] = {
                ...
                map_data: {
                    data: {
                        last_real_data: Array.from(new Array(range / 2 / data_frequency)).map((_, index) => {
                        ...
                        actual_prediction_lstm: Array.from(new Array(range / 2 / data_frequency)).map((_, index) => {
                            last_real_data = Math.sin(last_real_data) * 10000
                            return {
                        ...
                        actual_prediction_xgboost: Array.from(new Array(range / 2 / data_frequency)).map((_, index) => {
                            last_real_data = Math.sin(last_real_data) * 10000
                            return {
                                datetime: today + index * data_frequency,
                                load_consumption: last_real_data,
                        ...
                        ensemble: Array.from(new Array(range / 2 / data_frequency)).map((_, index) => {
                            last_real_data = Math.sin(last_real_data) * 10000
                            return {
                                datetime: today + index * data_frequency,
                                load_consumption: last_real_data,
                ...
                table_to_download: Array.from(new Array(range / data_frequency)).map((_, index) => {
                    last_real_data = Math.sin(last_real_data) * 10000
                    return {
                        datetime: yesterday + index * data_frequency,
                ...
                metrix_tables: {
                    XGBoost: {
                        metrics_table: Array.from(new Array(range / data_frequency)).map((_, index) => {
                            last_real_data = Math.sin(last_real_data) * 10000
                            return {
