		...
		>
			{[`minute`, `hour`, `day`,].map((sectionId, index) => (
				<li key={`section-${index}`}>
					<ul>
						<ListSubheader sx={{ bgcolor: 'background.paper', }}>{`${sectionId}:`.toUpperCase()}</ListSubheader>
						{[0, 1, 2, 3, 4].map((item) => (
...
export function SplitButton() {
	...
	const timezone_list = [`UTC`].concat(Array.from(new Array(10)).map(
		(_, index) => `GMT+${index < 9 ? '0' + (index + 1) : index + 1}`
	))
		...
		<React.Fragment>
			...
			>
				{({ TransitionProps, placement }) => (
					...
					>
						<Paper sx={{ marginLeft: `1.5rem`, marginTop: `1.5rem`, border: `1px solid black` }}>
							<ClickAwayListener onClickAway={handleClose}>
								<Box sx={{ p: 1 }}>
									...
									<Stack sx={{ padding: `1rem` }}>
										<Typography variant="caption">Coordinated Universal Time UTC, GMT UTC+00:00</Typography>
										<Stack direction={"row"} sx={{ alignItems: `center` }}>
											<Typography
												variant="button"
												color="textPrimary"
												sx={{ marginRight: `1rem` }}>timezone:</Typography>
											<IconButton
...
export function Navbar({
	...
	return (
		<Stack sx={{
			...
			<Button
				variant="outlined"
				sx={{ color: textColor, maxHeight: `40px`, background: `var(--mui-palette-common-white)` }}
			>Reset zoom</Button>
			<Button
				color="primary"
				variant="outlined"
			...
			<Stack direction={"row"} sx={{ alignItems: `center` }}>
				<Typography
					variant="button"
					color="textPrimary"
					sx={{ marginRight: `1rem` }}>refresh at</Typography>
				<IconButton
					onClick={handleClick2}
					sx={{
						color: textColor,
