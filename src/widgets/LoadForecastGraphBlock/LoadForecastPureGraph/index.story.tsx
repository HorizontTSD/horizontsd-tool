...
export const LoadForecastPureGraphStory: Story = {
    ...
    render: (local, global) => {
        const {
            data: sensorsList,
            error: sensorsList_error,
            isLoading: sensorsList_isLoading,
        } = useFuncGetSensorIdListBackendV1GetSensorIdListGetQuery()
        const [sensorIds, setSensorIds] = useState<string>(sensorsList?.join(", ") || "")
        const [triggerForecast, { data, isLoading, error }] =
            useFuncGetForecastDataBackendV1GetForecastDataPostMutation()
        ...
        return (
            <div>
                <div>
                    <h2>Sensor IDs</h2>
                    {sensorsList_isLoading ? (
                        <div>Loading...</div>
                    ) : sensorsList_error ? (
                        <div>Error loading sensors!</div>
                    ) : (
                        <ul>{sensorsList?.map((sensorId) => <li key={sensorId}>{sensorId}</li>)}</ul>
                    )}
                </div>
                <div>
                        ...
                        <div>
                            <h2>Forecast Data:</h2>
                            {
                                ...
                                data.map((e, i) => {
                                    ...
                                    return <LoadForecastPureGraph key={item.description.sensor_id} initialData={item} />
                                })
                            }
                        </div>
                    )}
