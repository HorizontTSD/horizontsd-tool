...
export const Metrix = () => {
  // Data fetching hooks
  const { data: rootData, isLoading: isRootLoading } =
    useReadRootGetQuery();
  ...
  const startDefaultDate = useRef(rootData?.start_default_date ? new Date(rootData.start_default_date) : new Date(Date.now() - 1000 * 60 * 60 * 24));
  ...
  const availableModels = metricsData && metricsData.length > 0
